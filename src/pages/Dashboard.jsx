  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok && result.success) {
        showToast(current._id ? "✓ Mis à jour" : "✓ Créé avec succès");
        fetchData();
        setModal(null);
      } else {
        showToast(`❌ ${result.error || "Erreur de sauvegarde"}`);
      }
    } catch (err) {
      showToast("❌ Connexion au serveur échouée");
    }
  };
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#FAF6F0] font-['Jost'] text-[#1a1008] overflow-hidden">
      ...
      `}>
        ...
        <nav className="flex-1 p-6 space-y-3 mt-16 md:mt-0">
          <div className="text-[10px] uppercase tracking-[0.2em] text-[#8a7060] mb-4 font-bold">Gestion</div>
      ...
      <main className="flex-1 overflow-y-auto relative pb-20">
        <header className="px-6 py-6 md:h-24 md:px-10 flex flex-col md:flex-row items-start md:items-center justify-between sticky top-0 bg-[#FAF6F0]/80 backdrop-blur-xl z-40 border-b border-[#F2EBE0] gap-4">
          <h2 className="font-['Playfair_Display'] text-2xl md:text-3xl font-black capitalize">
            {view === "dogs" ? "Inventaire Chiots" : "Bibliothèque des Races"}
          </h2>
          <button 
          ...
          >
            ＋ Ajouter {view === "dogs" ? "un Chiot" : "une Race"}
          </button>
        </header>

        <div className="p-6 md:p-10">
          {loading ? (
            <div className="text-center py-20 opacity-50 font-bold">Chargement...</div>
          ) : view === "dogs" ? (
                  ...
                  <div key={dog._id} className="bg-white rounded-[32px] p-4 shadow-sm border border-[#F2EBE0] group">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-4 pt-4 border-t border-dashed flex justify-between items-center">
                        <span className="text-[10px] text-[#8a7060] uppercase font-bold tracking-widest">{dog.color || "Couleur N/A"}</span>
                    ...
                    {breeds.map(b => (
                      <tr key={b._id} className="hover:bg-[#FAF6F0]/50 transition-colors">
                        ...
                        <td className="p-6 text-right space-x-4">
                          <button onClick={() => { setCurrent(b); setModal("form"); }} className="text-[#C1654A] font-bold text-xs uppercase tracking-widest">Éditer</button>
                          <button onClick={() => { setDeleteTarget(b._id); setModal("delete"); }} className="text-red-500 font-bold text-xs uppercase tracking-widest">Supprimer</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#1a1008]/60 backdrop-blur-md overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-[30px] md:rounded-[40px] shadow-2xl p-6 md:p-10 my-auto">
            <h2 className="text-2xl md:text-3xl font-['Playfair_Display'] font-black mb-6 md:mb-8">
                {current._id ? "Modifier" : "Ajouter"} {view === "dogs" ? "le Chiot" : "la Race"}
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-h-[60vh] overflow-y-auto px-2">
              {view === "dogs" ? (
                <>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Nom</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none border border-transparent focus:border-[#C1654A]" value={current.name} onChange={e => setCurrent({...current, name: e.target.value})} placeholder="Bella" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Race</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Âge</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none" value={current.age} onChange={e => setCurrent({...current, age: e.target.value})} placeholder="ex: 8 semaines" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Genre</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Couleur</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none" value={current.color} onChange={e => setCurrent({...current, color: e.target.value})} placeholder="ex: Fauve" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Prix ($)</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Statut</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Pédigrée</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none" value={current.pedigree} onChange={e => setCurrent({...current, pedigree: e.target.value})} placeholder="ex: AKC Certified" />
                  </div>
                </>
              ) : (
                <>
                  <div className="flex flex-col gap-2 md:col-span-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Nom de la Race</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none" value={current.title} onChange={e => setCurrent({...current, title: e.target.value})} placeholder="ex: Cavapoo" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Origine</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none" value={current.origin} onChange={e => setCurrent({...current, origin: e.target.value})} placeholder="ex: France" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[10px] font-black uppercase text-[#8a7060]">Tempérament</label>
                    <input className="p-4 bg-[#FAF6F0] rounded-2xl outline-none" value={current.temperament} onChange={e => setCurrent({...current, temperament: e.target.value})} placeholder="ex: Joueur" />
                  </div>
                </>
              )}

              <div className="md:col-span-2 flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-[#8a7060]">Média (Image)</label>
              ...
              <div className="md:col-span-2 flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-[#8a7060]">Description & Détails</label>
                <textarea rows="3" className="p-4 bg-[#FAF6F0] rounded-2xl outline-none border border-transparent focus:border-[#C1654A]" value={current.description} onChange={e => setCurrent({...current, description: e.target.value})} placeholder="Écrire les détails ici..."></textarea>
              </div>
            </div>

            <div className="mt-8 flex flex-col-reverse md:flex-row justify-end gap-4">
              <button onClick={() => setModal(null)} className="px-8 py-4 font-bold text-[#8a7060]">Annuler</button>
              <button onClick={handleSave} className="px-10 py-4 bg-[#C1654A] text-white font-bold rounded-2xl shadow-xl hover:bg-[#9E4A32] transition-all">Enregistrer</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modal === "delete" && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-[#1a1008]/80 backdrop-blur-md p-4">
           <div className="bg-white p-8 md:p-10 rounded-[32px] text-center max-w-sm w-full">
             ...
             <h3 className="text-xl font-bold mb-2">Supprimer définitivement ?</h3>
             <p className="text-sm text-[#8a7060] mb-8">Cette action est irréversible.</p>
             <div className="flex gap-3 flex-col sm:flex-row">
               <button onClick={() => setModal(null)} className="flex-1 py-3 font-bold text-[#8a7060] bg-[#FAF6F0] rounded-xl order-2 sm:order-1">Annuler</button>
               <button onClick={confirmDelete} className="flex-1 py-3 font-bold text-white bg-red-500 rounded-xl shadow-lg order-1 sm:order-2">Supprimer</button>
             </div>
           </div>
        </div>
      )}
